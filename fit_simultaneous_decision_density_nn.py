...
def parse_args(args):
    ...
    parser.add_argument('--support-sim-num',
        type=int,
        help="MAXIMUM number of points to sample from the support (uniformly)",
        default=100)
    ...
    parser.add_argument('--log-barrier-params',
        ...
        """,
        default="0.01")
    parser.add_argument('--inflation-params',
        type=str,
        help="""
        The weight param for the inflation penalty
        comma separated list (for tuning over)
        """,
        default="0.5")
    ...
    parser.add_argument('--batch-size',
        type=int,
        help="mini batch size for optimizaiton",
        default=4000)
    ...
    args.inflation_params = process_params(args.inflation_params, float)
...
def main(args=sys.argv[1:]):
    ...
    param_grid = [{
        ...
        'inflation_param': args.inflation_params,
        ...
        'dec_embedder': [data_dict["train"].dec_embedder if hasattr(data_dict["train"], "dec_embedder") else None],
    ...
    fitted_model, best_hyperparams, cv_results = do_cross_validation(
        ...
        cv=args.cv)
    total_train_time = time.time() - st_time
    logging.info("Best hyperparams %s", best_hyperparams)
    logging.info("Total train time %f", total_train_time)
