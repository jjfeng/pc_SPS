...
def plot_results(
    ...
    for res_dict in results:
        for model_idx in range(len(res_dict["loss"])):
            ...
            all_data += extract_row(accept_prob, recalib_ci[0], "coverage", "recalib_guard_indiv")
            all_data += extract_row(accept_prob, recalib_ci[1], "coverage", "recalib_guard_indiv")
        ...
        all_data += extract_row(accept_prob, recalib_agg_ci[0], "coverage", "recalib_guard_agg")
        all_data += extract_row(accept_prob, recalib_agg_ci[1], "coverage", "recalib_guard_agg")
        all_data += extract_row(accept_prob, res_dict["test_agg_coverage"][args.alpha], "coverage", "test_agg")


    data_df = pd.DataFrame(all_data)
    print(data_df)
    logging.info(data_df.to_latex(formatters={"value": lambda x: "%.3f" % x}))
