...
def curvy_1d_sigma(xs):
    return np.ones(xs.shape[0]) * 0.1
...
def simple_surprise_2d_sigma(xs):
    return 0.3 * np.ones(xs.shape[0])

def high_dim_mu(xs):
    return simple_2d_mu(xs) + 0.5 * simple_2d_mu(xs[:,2:]) * simple_2d_mu(xs[:,4:])

def high_dim_sigma(xs):
    sigma = 0.5 * simple_2d_sigma(xs) + simple_2d_sigma(xs[:,2:]) * simple_2d_sigma(xs[:,4:])
    return sigma

def constant_high_dim_mu(xs):
    return simple_2d_mu(xs) + 0.5 * simple_2d_mu(xs[:,2:]) * simple_2d_mu(xs[:,4:])

def constant_high_dim_sigma(xs):
    return np.ones(xs.shape[0])
