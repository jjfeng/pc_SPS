...
def plot_accepted_vs_entropy(data_dict, fitted_models, args):
    """
    ...
    print(data_dict["support_sim_settings"].num_p)
    print("SHAPE", test_x.shape)
    y_given_x_sigma = data_dict["data_gen"].sigma_func(test_x)
    entropy = get_normal_dist_entropy(y_given_x_sigma)

    all_pred_dfs = []
    for fitted_model in fitted_models:
        print(fitted_model.cost_decline)
        x_accept_probs = fitted_model.get_accept_prob(test_x).ravel()
        pred_df = pd.DataFrame({"entropy": entropy})
        pred_df["accept"] = x_accept_probs.ravel()
        pred_df["delta"] = fitted_model.cost_decline
        all_pred_dfs.append(pred_df)
        print(pred_df.accept.mean())
        print(entropy.mean(), entropy.max(), entropy.min())
        print("ENT rej", entropy[x_accept_probs < 0.1].mean())
        print("ENT rej", entropy[x_accept_probs < 0.1].std())
        print("ENT acc", entropy[x_accept_probs > 0.8].mean())
        print("ENT acc", entropy[x_accept_probs > 0.8].std())

    plt.clf()
    all_pred_dfs = pd.concat(all_pred_dfs)
    sns.lmplot(x="accept", y="entropy", hue="delta", data=all_pred_dfs, lowess=True, scatter=True)
...
def plot_accepted_rejected_region(data_dict, fitted_models, args, mesh_size=0.05):
    ...
    all_accept_probs = []
    for fitted_model in fitted_models:
        print("COST DECLINE", fitted_model.cost_decline)
        x_accept_probs = fitted_model.get_accept_prob(mesh_coords).ravel()
    ...
    for idx, x_accept_probs in enumerate(all_accept_probs):
        cs = ax.contour(
                ...
                linewidths=4)
        ax.clabel(cs, inline=1, fontsize=10, fmt=str(fitted_models[idx].cost_decline))
