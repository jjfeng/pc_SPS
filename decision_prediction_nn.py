...
class DecisionPredictionNNs(DecisionPredictionBaseModel):
    ...
    def _create_decision_model(self, max_sigmoid_scale: float=100.0, init_scale: float = 0.5):
        ...
        pen_accept_prob = accept_prob

        # With an additional decision network?
        if self.decision_layer_sizes is not None:
            if self.has_dec_embedder:
                self.dec_embed_concat_ph = tf.placeholder(tf.float32, [None, self.dec_embedder.n_components], name="dec_embed")
            print("CREATING DECISION NN")
            assert int(self.decision_layer_sizes[-1]) == 1
            decision_nn = NeuralNetwork.create_full_nnet(
                    self.decision_layer_sizes,
                    self.dec_embed_concat_ph if self.has_dec_embedder else self.x_concat_ph,
            ...
            accept_prob = decision_nn.layers[-1] * accept_prob
            pen_accept_prob = decision_nn.layers[-1]
        #else:
        #    scale = tf.Variable(init_scale/self.cost_decline, dtype=tf.float32, name="dec0")
        #    scale1 = tf.Variable(0, dtype=tf.float32, name="dec1")
        #    decision_model_params = [scale, scale1]
        ...
        return decision_model_params, accept_prob, pen_accept_prob

    def _setup_optimization_problem(self):
        self.decision_model_params, self.accept_prob, self.accept_prob_to_pen = self._create_decision_model()
        ...
        self.weight_decision = tf.add_n([tf.nn.l2_loss(w) for w in self.decision_model_params])
        ...
        self.accept_support_proportion = tf.reduce_sum(self.accept_prob_to_pen * (1.0 - self.is_data_ph))/tf.reduce_sum(1.0 - self.is_data_ph)
        ...
        self.loss = self.prediction_loss_all * self.inflation_param + self.prediction_loss + self.tot_cost_reject
        self.actual_loss = self.prediction_loss + self.tot_cost_reject
        if self.do_no_harm_param > 0 and self.support_sim_num > 0:
            # The extra (inflation_param + 1) is because we should scale the other penalties based on inflated accept all loss and the usual empirical loss.
            self.pen_loss = self.loss + (self.inflation_param + 1) * (self.weight_pen + self.accept_pen + self.log_barrier)
        else:
            self.pen_loss = self.loss + (self.inflation_param + 1) * (self.weight_pen + self.log_barrier)
    ...
    def fit(self, X, y, weights=None, check_accept_prob_thres:float = 0):
        """
        ...
        with sess.as_default():
            ...
            for n_init in range(self.num_inits * 3):
                ...
                tot_inits += int(np.isfinite(train_loss) * (accept_data > check_accept_prob_thres))
    ...
    def _fit_one_init(self, sess, x_train, y_train, log_show=50, stop_thres=1, stop_ratio=1e-20, min_iters=1000):
        ...
        if self.do_no_harm_param > 0 and self.support_sim_num > 0:
            return self._fit_one_init_with_sim(
                    sess,
                    self.pen_loss,
                    self.train_op_round2,
                    x_train,
                    y_train,
                    log_show=log_show,
        ...
        for i in range(self.max_iters):
            ...
            for j in range(num_sgd_slices):
                ...
                if inner_idx % log_show == 0:
                    ...
                    logging.info("train %d: (epoch %d), avg_accept %f", inner_idx, i, np.mean(accept_prob))
    ...
    def _fit_one_init_with_sim(self, sess, loss_node, train_op, x_train, y_train, log_show=50, stop_thres=1, stop_ratio=1e-8, min_iters=100):
        ...
        if num_obs > self.sgd_batch_size * num_sgd_slices:
            num_sgd_slices += 1
        print("sgd slices", num_sgd_slices, "num obs", num_obs,"batch size", self.sgd_batch_size)
        ...
        for i in range(self.max_iters):
            support_sim_num = int(min(
                ...
                50.0/(accept_unif_proportion + 0.005)))
            random_support = self.support_sim_settings.support_unif_rvs(support_sim_num * num_sgd_slices)
            if num_sgd_slices > 1:
                shuffled_indices = np.random.permutation(np.arange(num_obs))
                x_train_shuffled = x_train[shuffled_indices]
                y_train_shuffled = y_train[shuffled_indices]
            ...
            aux_train_shuffled = self.dec_embedder.transform(
                    x_train_shuffled.reshape((x_train_shuffled.shape[0], -1))) if self.has_dec_embedder else None
            ...
            for j in range(num_sgd_slices):
                ...
                y_filler = np.ones((support_sim_num, self.label_size)) * y_train_sgd[0]
                ...
                random_support_sgd = random_support[support_sim_num * j: support_sim_num * (j + 1)]
                if self.has_dec_embedder:
                    dummy_x = np.concatenate([x_train_shuffled[:1]] * support_sim_num)
                    x_concat = np.concatenate([x_train_sgd, dummy_x])
                    aux_train_sgd = aux_train_shuffled[start_idx:stop_idx]
                    dec_embed_concat = np.concatenate([aux_train_sgd, random_support_sgd])
                    feed_dict = {
                        self.x_concat_ph: x_concat,
                        self.dec_embed_concat_ph: dec_embed_concat,
                        self.y_concat_ph: y_concat,
                        self.is_data_ph: is_data,
                     }
                else:
                    x_concat = np.concatenate([x_train_sgd, random_support_sgd])
                    dec_embed_concat = None
                    feed_dict = {
                        self.x_concat_ph: x_concat,
                        self.y_concat_ph: y_concat,
                        self.is_data_ph: is_data,
                ...
                _, acc_prob, tot_loss, all_loss, pred_loss, cost_reject, accept_unif_proportion, accept_pen, log_barr, accept_is_data_proportion, actual_loss = sess.run(
                    [
                        train_op,
                        self.accept_prob,
                        loss_node,
                ...
                if inner_idx % log_show == 0:
                    self._print_train(sess, x_train, y_train)
                    logging.info(
                            "iter %d (epoch %d): all-loss %f, pred-loss %f, cost reject %f, do no harm %f, log barr %f",
                            inner_idx, i, all_loss, pred_loss, cost_reject, accept_pen, log_barr)
                ...
                if i > self.max_iters/4:
                    if do_exit or accept_is_data_proportion < 0.000001:
                        break
    ...
    def get_accept_prob(self, x, max_replicates=100):
        ...
        with sess.as_default():
            ...
            num_replicates = max_replicates if self.dropout_rate > 0 else 1
            all_accept_probs = []
            feed_dict = {self.x_concat_ph: x}
            if self.has_dec_embedder and self.dec_embed_concat_ph is not None:
                trans_x = self.dec_embedder.transform(x.reshape((x.shape[0], -1)))
                print(trans_x)

                feed_dict[self.dec_embed_concat_ph] = self.dec_embedder.transform(x.reshape((x.shape[0], -1)))
            for i in range(num_replicates):
