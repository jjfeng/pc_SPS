...
def parse_args(args):
    ...
    parser.add_argument('--eps',
        ...
        default=0.1)
    parser.add_argument('--threshold-adjust',
        type=float,
        help="threshold adjust",
        default=0)
    ...
    parser.add_argument('--ulm-files',
        type=str,
        default="")
    parser.add_argument('--borrow-only-files',
        type=str,
        default="")
    ...
    args.borrow_only_files = process_params(args.borrow_only_files, str)
    args.plain_files = process_params(args.plain_files, str)
    return args

def load_all_models(args, train_data):
    ulm_models = {
            "ulm_%d" % idx: load_model(ulm_file)
            for idx, ulm_file in enumerate(args.ulm_files)}
    borrow_only_models = {
            "borrow_only_%d" % idx: load_model(borrow_only_file)
            for idx, borrow_only_file in enumerate(args.borrow_only_files)}
    raw_plain_models = [load_model(plain_file) for plain_file in args.plain_files]
    plain_models = {
            "plain_%d" % idx: DecisionPredictionModel(
                m,
                args.cost_decline,
                threshold=args.cost_decline - args.threshold_adjust)
            for idx, m in enumerate(raw_plain_models)}
    od_models = {
            "od_%d" % idx:
            EntropyOutlierPredictionModel(
                plain_models["plain_%d" % idx],
                args.cost_decline,
                threshold=args.cost_decline - args.threshold_adjust,
                eps=args.eps)
            for idx, m in enumerate(raw_plain_models)}
    for m in od_models.values():
        m.fit_decision_model(train_data.x)

    return {
            "ulm": ulm_models,
            "borrow_only": borrow_only_models,
            "plain": plain_models,
            "od": od_models}
...
def get_data(model_list, young_age_test_data, old_age_test_data, label):
    print(label)
    res_data = []
    for m_label, m in model_list.items():
        if m is None:
            logging.info("%s %s NOT AVAIL", label, m)
            continue
        ...
        print("NUM OB", old_age_test_data.num_obs)
        print(accept_prob)
