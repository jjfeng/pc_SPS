...
def assess_local_agg_coverage_true(
    ...
    res = {
            "local_coverage_mse": {
                "independent": [np.mean(np.power(accepted_agg_cov - (1 - pi_alpha), 2))],
                "individual": [np.mean(np.power(indiv_local_cov - (1 - pi_alpha), 2)) for indiv_local_cov in individual_coverages],
            },
            "local_coverage_var": {
                "independent": [np.var(accepted_agg_cov)],
                "individual": [np.var(indiv_local_cov) for indiv_local_cov in individual_coverages],
            },
            "local_coverage_iqr": {
                "independent": [scipy.stats.iqr(accepted_agg_cov)],
