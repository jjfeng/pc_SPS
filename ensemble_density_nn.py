...
class EnsembleDensityNN(EnsemblePredictionNN):
    ...
    def get_cond_dists(self, x, max_replicates=100):
        ...
        with sess.as_default():
            ...
            if self.density_parametric_form == "gaussian":
                ...
                #print("FINAL FINAL SIGMA", final_final_sigma)
    ...
    def get_embedding(self, x, max_replicates=100):
        sess = tf.Session()
        with sess.as_default():
            all_embedding = 0
            for pred_nn in self.nns:
                pred_nn._init_network_variables(sess)
                embedding = sess.run(
                    pred_nn.prediction_embedding,
                    feed_dict={
                        pred_nn.x_concat_ph: x})
                all_embedding += embedding
            mean_embedding = all_embedding/len(self.nns)
