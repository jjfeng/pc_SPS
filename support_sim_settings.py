...
class SupportSimSettingsInvPCA(SupportSimSettings):
    """
    Here we specify separate min and max for each pca dimension
    Generate random PC vectors uniformly
    Then we inverse transform random PC vectors to the original image space
    """
    def __init__(self, pca, orig_shape: List[int], num_p: int, min_x: ndarray, max_x: ndarray):
        self.pca = pca
    ...
    def support_unif_rvs(self, n: int):
        ...
        @return random vectors drawn from the support uniformly
        """
        rand_pcs = np.random.rand(n, self.num_p) * (self.max_x - self.min_x) + self.min_x
        inv_xs = self.pca.inverse_transform(rand_pcs)
        inv_xs = inv_xs.reshape([n] + self.orig_shape)
        return inv_xs
    ...
    def check_obs_x(self, x):
        ...
        @return whether all points are within the support
        """
        new_x = self.pca.transform(x.reshape((x.shape[0], -1)))
        num_in_support = np.sum(
                (new_x >= self.min_x) * (new_x <= self.max_x),
                axis=1)
        in_support = num_in_support == self.num_p
